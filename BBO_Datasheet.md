# Datasheet — Query History and Function Evaluations Dataset

> BBO Capstone Project | Imperial College Professional Certificate in Machine Learning and AI
...
- All input values are continuous and bounded between 0 and 1, represented to six decimal places
- Output values are scalar real numbers
- All tasks are framed as maximisation problems; where the underlying objective is naturally a minimisation (such as side effects in Function 3), the output has been negated so that higher values consistently indicate better performance
- The dataset does not contain personally identifiable information
- There are no missing values in the output records, though some functions returned near-zero outputs during early rounds due to poor initialisation — this is noted as a known gap in early-round coverage of the search space

---

## Collection Process

Queries were generated through an iterative, strategy-driven process across ten biweekly rounds. In each round, input arrays were constructed based on Gaussian Process model outputs and acquisition function evaluations. The specific strategy evolved across rounds as follows:
...
- Analysis of optimisation strategy effectiveness across rounds
- Benchmarking of different acquisition function behaviours across function types
- Educational demonstration of Bayesian optimisation principles in practice

### Uses to Avoid

- Training general-purpose machine learning models
- Drawing statistical inferences about the broader population of black-box functions
- Any application outside the specific BBO challenge context for which this dataset was created
...
| Field | Detail |
|-------|--------|
| **Location** | GitHub repository associated with this capstone project |
| **Access** | Publicly accessible to course peers and facilitators |
| **Terms of use** | Imperial College programme terms; no additional licensing restrictions apply |
| **Maintained by** | Student author of the capstone project |
| **Update process** | Manual; updates applied directly to the repository with version notes in the commit history |
...
|----------|-------------|
| [Model Card](../model_card/BBO_Model_Card.docx) | Documents the Gaussian Process Bayesian Optimisation approach |
| [Project README](../README.md) | Full project overview and repository structure |
