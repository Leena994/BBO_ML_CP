# Model Card — Adaptive Gaussian Process Bayesian Optimisation
...
## Strategy and Techniques Across Rounds
...
Transparency supports reproducibility and adaptation in the following ways:

- The full query history and output records are preserved in the project repository, enabling any peer or facilitator to replicate the optimisation trajectory from any round
- Strategy decisions are documented per round so that the reasoning behind each submission can be reconstructed and evaluated independently
- The adaptive nature of the approach — where strategy changes are driven explicitly by observed data — means that the decision logic is auditable at each step rather than opaque
...
|----------|-------------|
| [Datasheet](../datasheet/BBO_Datasheet.docx) | Documents the query history and function evaluation dataset |
